...
const v8UtilClass = process.binding('atom_common_v8_util').v8Util;
...
var WebViewImpl = (function () {
    function WebViewImpl(webviewNode) {
        ...
        var self = this;
        var loopRun = setInterval(function () {
            if (!("native_setId" in (self.browserPluginNode)))
                return;
    ...
    WebViewImpl.prototype.onDomReady = function () {
        this.browserPluginNode.native_setId(this.browserPluginNode.idNum);
        var src = this.webviewNode.getAttribute(webViewConstants.ATTRIBUTE_SRC);
        this.browserPluginNode.native_loadURL(src, "", "", "");

        var preloadUrl = this.webviewNode.getAttribute(webViewConstants.ATTRIBUTE_PRELOAD);
        this.browserPluginNode.native_setPreloadURL(preloadUrl);

        var self = this;
        this.browserPluginNode.onNativeMessage = function (channel, ...args) {
            //console.log("this.browserPluginNode.onNativeMessage:" + channel);
            var domEvent = new Event("ipc-message");
            if ("ipc-message" != channel)
                domEvent = new Event(channel);
            domEvent.channel = channel;
            domEvent.args = JSON.stringify(args);
            self.browserPluginNode.dispatchEvent(domEvent)
        }
    }
    ...
    WebViewImpl.prototype.handleBrowserPluginAttributeMutation = function (attributeName, oldValue, newValue) {
        if (attributeName === webViewConstants.ATTRIBUTE_INTERNALINSTANCEID && !oldValue && !!newValue) {
            ...
            var self = this;
            webFrame.on("webviewResize", function (event, id, newSizeWidth, newSizeHeight) {
                self.onElementResize(newSizeWidth, newSizeHeight);
            });
...
//             return guestViewInternal.attachGuest(this.internalInstanceId, this.guestInstanceId, this.buildParams());
    ...
    WebViewImpl.prototype.onElementResize = function (newSizeWidth, newSizeHeight) {
        ...
        // is incorrect
        newSizeWidth = this.webviewNode.clientWidth;
        newSizeHeight = this.webviewNode.clientHeight;

        resizeEvent.newWidth = newSizeWidth;
        resizeEvent.newHeight = newSizeHeight;
        this.dispatchEvent(resizeEvent);
        if (this.guestInstanceId) {
//             return guestViewInternal.setSize(this.guestInstanceId, {
//                 normal: newSize
//             });
            this.browserPluginNode.native_setSize(newSizeWidth, newSizeHeight);
        }
    }
...
//         });
    ...
    WebViewImpl.prototype.buildParams = function () {
        ...
        for (attributeName in ref1) {
            if (!hasProp.call(ref1, attributeName))
                continue;
...
var registerBrowserPluginElement = function () {
    ...
    proto.createdCallback = function () {
        ...
        this.setAttribute('id', 'browser-plugin-' + this.idNum);
...
var registerWebViewElement = function () {
    ...
    proto.loadURL = function (url, options) {
        ...
        var extraHeaders = "";
        if (options) {
            if (("httpReferrer" in options))
                httpReferrer = options.httpReferrer;
            if (("userAgent" in options))
                userAgent = options.userAgent;
            if (("extraHeaders" in options))
                extraHeaders = options.extraHeaders;
        }
        var internal = v8Util.getHiddenValue(this, 'internal');
        if (!internal)
            return;
        internal.browserPluginNode.native_loadURL(url, httpReferrer, userAgent, extraHeaders);
    }
    ...
    createBlockHandler = function (m) {
        return function (...args) {
            ...
            if (internal && internal.browserPluginNode) {
                return internal.browserPluginNode["native_" + m].apply(internal.browserPluginNode, args);
            } else {
   ...
   createNonBlockHandler = function (m) {
        return function (...args) {
            const internal = v8Util.getHiddenValue(this, 'internal');
            if (internal && internal.browserPluginNode) 
                internal.browserPluginNode["native_" + m].apply(internal.browserPluginNode, args);
            else
                throw new Error(`Cannot call ${m} because the webContents is unavailable! The WebView must be attached to the DOM and the dom-ready event emitted before this method can be called.`);
    ...
    proto.executeJavaScript = function (code, hasUserGesture, callback) {
        ...
        if (undefined == hasUserGesture)
            hasUserGesture = false;

        var retVal = internal.browserPluginNode.native_executeJavaScript(code, hasUserGesture);
        if ("undefined" == retVal && callback) {
            callback(undefined);
            return
        }
        if (callback) {
            var retJson = JSON.parse(retVal);
            callback(retJson.ret);
        }
    }
